...
for i in range(m, m+1):
    ...
    output_file = "/Users/roumyadas/Desktop/IPL_Simulation/Match_flows/ball-by-ball_" + str(m+1) + ".txt"
        ...
        with redirect_stdout(f):
            ...
            def calculate_bowler_probabilities(bowlers_df, overs_bowled, last_over_bowled, over, wickets_taken,runs_conceeded_over):
                ...
                for _, row in bowlers_df.iterrows():
                    ...
                    ####
                    if over <= 4 and rank <= 2:
                        prob += 0.35
                        
                    if over <= 4 and rank > 3:
                        prob *= 0.3
                        
                    if rank <= 2 and over >4 and over < 6:
                        prob += 0.2
                    ####   
