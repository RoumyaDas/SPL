...
def match_scorecard(match_id, df_all):
    ...
    output_file = "/Users/roumyadas/Desktop/IPL_Simulation/Season_03/Scorecards/matchcard_" + str(match_id) + ".txt"
    ...
    with open(output_file, "w") as f:
        with redirect_stdout(f):
            ...
            venue = teams[0]  ##update for playoffs
            #venue = 'PBKS' 

            print(f"Match between {teams[0]} and {teams[1]} !! ")
            print(f"Venue -- {venue}")
            ...
            for player in bat_1st_team:
                if player in batter_stats['striker'].values:
                    ...
                    fours = bat_info['fours'].iloc[0] if not bat_info.empty else 0
                    sixes = bat_info['sixes'].iloc[0] if not bat_info.empty else 0
            ...
            batter_stats_mod['4s_6s'] = batter_stats_mod[['4s','6s']].apply(lambda x: f"4s = {int(x[0])}, 6s = {int(x[1])}", axis=1)
            ######################################################
            ...
            ###############
            batter_stats_mod = batter_stats_mod[['player','wicket_event','runs','balls','strike_rate','4s_6s']]
            ...
            batter_stats_mod_2['4s_6s'] = batter_stats_mod_2[['4s','6s']].apply(lambda x: f"4s = {int(x[0])}, 6s = {int(x[1])}", axis=1)
            ...
            ###############
            batter_stats_mod_2 = batter_stats_mod_2[['player','wicket_event','runs','balls','strike_rate','4s_6s']]
