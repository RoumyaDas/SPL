...
def generate_bowling_order(bowler_dict, max_overs=4):
    ...
    for over in range(2, 20):
        ...
        primary = [b for b in eligible if bowler_ranks[b] <= 5]
        backup = [b for b in eligible if bowler_ranks[b] > 5]
...
bat_stat = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='Bat')
bowl_stat = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='Bowl')
bat_phase = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='BAT_phase')
bowl_phase = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='BOWL_phase')
h2h_ = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='h2h')
pace_stat = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='pace')
spin_stat = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='spin')
form_bat = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='form_bat')
form_ball = pd.read_excel('/Users/roumyadas/Desktop/IPL_Simulation/Season_03/updates_/separate_stats.xlsx', sheet_name='form_bowl')
...
for i in range(m, m+1):
    ...
    pitch = ('spin' if ground_factor>1.5 else 'pace' if ground_factor<=0.66 else 'neutralish')
    ...
    output_file = "/Users/roumyadas/Desktop/IPL_Simulation/Season_03/Match_flows/ball-by-ball_" + str(m+1) + ".txt"
    #output_file = "/Users/roumyadas/Desktop/IPL_Simulation/experimentation/ball-by-ball_" + str(m+1) + ".txt"
    ...
    with open(output_file, "w") as f:
        with redirect_stdout(f):
            ...
            m_id = 'S03M00' + str(i+1) if i<9 else 'S03M0' + str(i+1)
            ...
            if len(bowling_order)<=10:
                ...
                # Main loop
                while True:
                    sequence = generate_bowling_order(bowler_dict, max_overs=4)
            ...
            for over in range(1, 21):
                ...
                for ball in range(1, 10):  # Accounting for max 10 balls in case of extras in each over
                    ...
                    #rpo based
                    rpo_factor = (12/rpo if over >=2 else 1)
                    sd = 0.025 #0.15
                    ...
                    final_probs = {
                        ...
                        'wicket': max(0.01,np.random.normal(0, 0.075) + (ground_probs['wkt_prob'] + batter_probs['out_prob'] + bowler_probs['wkt_prob']) * (form_factor_bowl.get(bowler, 1)*np.random.normal(1,0.1)) * (h2h_bowl[0] if len(h2h_bowl)>0 else 1) / 3) * wkt_factor * free_hit_factor*l_o_out_factor * g_factor * (1/bowl_type_factor[0] if len(bowl_type_factor)>0 else 1),
                        '0+runout': (0.003)/3,
                        '1+runout': (0.0025)/3,
                        '2+runout': (0.002)/3,
                        '3+runout': 0.0002,
                    ...
                    fielder= ''
                    if event == 'wicket':
                        wkt_type = random.choices(['Caught', 'LBW','Bowled', 'Stumped/Runout by Keeper', 'HitWicket'], weights=[0.6, 0.19, 0.19, 0.01, 0.01])[0]
            ...
            print("chasing team's batters are coming out!!")
            ...
            for over in range(1, 21):
                ...
                for ball in range(1, 10):  # Accounting for max 10 balls in case of extras in each over
                    ...
                    last_3_ov_factor = (1.15 if (last_3_ov_runs<=27 and last_3_ov_wkts<=1) else 0.95 if (last_3_ov_wkts>=2) else 1)
                    ...
                    if ball>=2:
                        free_hit_factor_bdry = (3 if event=='noball' else 1)
                    st_ro_factor = (2 if over>=16 else 1)
                    rr_factor = 1.33#(0.75 if reqd_run_rate<=8 else 1.33 if reqd_run_rate>12 else 1)
                    sd = 0.025 #0.15
                    ...
                    fielder= ''
                    if event == 'wicket':
                        wkt_type = random.choices(['Caught', 'Bowled', 'LBW', 'Stumped/Runout by Keeper', 'HitWicket'], weights=[0.6,0.19, 0.19, 0.01, 0.01])[0]
                        #row.update({'wicket_event': wkt_type})
                        if wkt_type == 'Caught':
                            weights = [0.075] * 10 + [0.25]
                            # Randomly choose a fielder based on weights
                    ...
                    if (legal_balls % 6 == 0)&(legal_balls!=legal_balls_last):
                        ...
                        #print(fixed_order[over-1], bowlers_df[['Func_Name','bowl','phase']])
    ...
    df_mod['pitch'] = pitch
    df_mod['toss'] = toss_win_team
    df_mod['decision'] = decision
...
tied_matches = df_match_info[df_match_info.winner=='TIE']['match_id'].unique().tolist()
...
for m__ in tied_matches:
    team_0 = df_all[df_all.match_id==m__]['batting_team'].unique()[0]
    team_1 = df_all[df_all.match_id==m__]['batting_team'].unique()[1]
    t0_pts = pts_table[pts_table.team==team_0]['points'].unique()[0]
    t1_pts = pts_table[pts_table.team==team_1]['points'].unique()[0]
    pts_table.loc[pts_table.team==team_0, 'points'] = t0_pts+1
    pts_table.loc[pts_table.team==team_1, 'points'] = t1_pts+1

pts_table = pts_table[pts_table.team!='TIE'].reset_index(drop=True)
